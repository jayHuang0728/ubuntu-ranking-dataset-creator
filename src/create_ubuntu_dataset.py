...
def translate_dialog_to_lists(dialog_filename):
    """ Translates a dialog into a list of lists of utterances.

    In the first list each item holds subsequent utterances from the same user.
    The second level list holds the individual utterances.
    ...
    with open(dialog_filename, 'r') as fin:
        dialog_reader = unicode_csv_reader(
...
def get_random_utterances_from_corpus(candidate_dialog_paths, rng,
                                      utterances_num=9, min_turn=3, max_turn=20):
    """ Sample multiple random utterances from the whole corpus.

    Args:
      candidate_dialog_paths:
      rng:
      utterances_num: number of utterances to generate
      min_turn: minimal index of turn that the utterance is selected from
    """
    utterances = []
    dialogs_num = len(candidate_dialog_paths)

    for i in range(0, utterances_num):
        # sample a random dialog
        ...
        if dialog_len < min_turn:
            print("Dialog {} was shorter than the minimum required length {}".format(
                dialog_path, dialog_len))
            exit()
        ...
        utterance = single_user_utterances_to_string(dialog[turn_index])
        utterances.append(utterance)
    return utterances


def single_user_utterances_to_string(utterances_list):
    """
...
def dialog_turns_to_string(dialog):
    ...
    """
    # join utterances
    turns_as_strings = list(map(single_user_utterances_to_string, dialog))
    # join turns
...
def create_random_context(dialog, rng, minimum_context_length=2, max_context_length=20):
    ...
    Returns:
      context: index of next utterance that follows the context
    """
...
def create_single_dialog_train_example(context_dialog_path, candidate_dialog_paths, rng, positive_probability,
    ...
    if positive_probability > rng.random():
        ...
        response = single_user_utterances_to_string(dialog[next_utterance_ix])
        label = 1.0
    else:
...
def create_single_dialog_test_example(context_dialog_path, candidate_dialog_paths, rng, distractors_num, max_context_length):
    ...
    # use the next utterance as positive example
    positive_response = single_user_utterances_to_string(
...
def prepare_data_maybe_download(directory):
    """Download ubuntu_dialogs.tgz file from cs.mcgill.ca/ and unpack it if dialogs/ doesn't exist. """
    archive_filename = 'ubuntu_dialogs.tgz'
...
if __name__ == '__main__':
    ...
    def create_eval_dataset(args, file_list_csv):
        ...
        dialog_paths = [os.path.join(args.data_root, "dialogs", path)
                        for path in convert_csv_with_dialog_paths(f)]

        def creator_function(context_dialog, candidates):
            return create_single_dialog_test_example(context_dialog, candidates, rng, args.n, args.max_context_length)

        # output the dataset
        data_set = create_examples(dialog_paths,
                                   examples_num=len(dialog_paths),
        ...
        for row in data_set:
            ...
            if args.tokenize:
                ...
                if args.stem:
                    translated_row = [list(map(stemmer.stem, sub))
                                      for sub in translated_row]
                if args.lemmatize:
                    translated_row = [[lemmatizer.lemmatize(
                        tok, pos='v') for tok in sub] for sub in translated_row]
    ...
    parser_train.add_argument('-e', '--examples', type=int,
                              default=1000000, help='number of examples to generate')
    parser_train.set_defaults(func=train_cmd)

    parser_test = subparsers.add_parser('test', help='test set generator')
    ...
    parser_valid = subparsers.add_parser('valid', help='validation set generator')
